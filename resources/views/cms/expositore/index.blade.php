@extends('layouts.app')
@section('content')
<h1 class="titulo-index">ADMINISTRADOR DE EXPOSITORES PARA EL EVENTO: <b>{{$evento->nombre}}</b></h1>
    <hr>
    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                ...
                <a href="{{route('expositore.create',$evento->id)}}" class="btn btn-primary btn-sm"><i class="icofont-plus-square"></i> Agregar nuevo</a>
            </div>
        </div>
    </div>
                        ...
                        @if (!$model->isEmpty())
                            ...
                            @foreach ($model as $item)
                                ...
                                <tr>
                                    ...
                                    <td>{{$item->estado}}</td>
                                    <td>
                                        <a  data-toggle="tooltip" data-placement="top" title="Editar {{$item->nombre}}"  href="{{route('expositore.edit',$item)}}" class="btn btn-warning btn-sm"><i class="icofont-edit"></i>  </a>
                                    <a data-toggle="tooltip" data-placement="top" title="Eliminar elemento" onclick = "if (! confirm('¿Estás seguro que deseas eliminar? Es probable que esta información deje de aparecer para los usuarios')) { return false; }" 
                                             href="{{route('expositore.eliminar',$item)}}" class="btn btn-danger btn-sm"><i class="icofont-trash"></i> </a>
